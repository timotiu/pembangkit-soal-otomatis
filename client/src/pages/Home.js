...
const [tipeSoal,setTipeSoal]=useState('');
  ...
  const res=await axios.post("http://localhost:3001",{nomor:e.target.parentNode.nextElementSibling.value,inputSoal,tipeSoal});
...
const hoverTooltip=(e)=>{
  ...
  for(let i=1;i<11;i++){
      ...
      <input defaultValue={"C3"}  onChange={(e)=>{setTipeSoal(e.target.value)}} id={'soal'+i} className="w-9 text-white bg-[#ac2020] border-[1px] border-gray-400 rounded-md pl-2 text-wrap" type="text"/>
